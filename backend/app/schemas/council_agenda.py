...
class CouncilAgendaBase(BaseModel):
    ...
    materials_url: Optional[str] = Field(
        None, max_length=2048, description="資料URL（レガシー用）"
    )
...
class CouncilAgendaOut(CouncilAgendaBase):
    ...
    id: UUID
    meeting_id: UUID
    materials_processing_status: str = Field(
        default="pending", description="資料処理状態（レガシー用）"
    )
    minutes_processing_status: str = Field(
        default="pending", description="議事録処理状態"
    )
    has_materials_summary: bool = Field(
        default=False, description="資料要約生成済み（レガシー用）"
    )
    has_minutes_summary: bool = Field(default=False, description="議事録要約生成済み")
    materials_count: int = Field(default=0, description="登録資料数")
    materials: List[CouncilAgendaMaterialOut] = Field(
        default=[], description="資料リスト"
    )
    created_at: datetime
    updated_at: datetime
...
class CouncilAgendaDetailOut(BaseModel):
    ...
    materials_text: Optional[str] = Field(
        None, description="取得した資料テキスト（レガシー用）"
    )
    ...
    processing_error: Optional[str] = Field(None, description="処理エラーメッセージ")
    materials_count: int = Field(default=0, description="登録資料数")
    materials: List[CouncilAgendaMaterialDetailOut] = Field(
        default=[], description="資料リスト（詳細）"
    )
    created_at: datetime
    updated_at: datetime
