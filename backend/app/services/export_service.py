...
class ExportService:
            ...
            "messages": [
                {
                    ...
                    "content": msg.content,
                    "source_refs": (
                        json.loads(msg.source_refs) if msg.source_refs else None
                    ),
                    "created_at": (
    ...
    def format_email_markdown(
        self, email: Any, notebook_title: str | None = None
    ) -> str:
    ...
    def format_minute_markdown(
        self, minute: Any, notebook_title: str | None = None
    ) -> str:
    ...
    def format_notebook_markdown(
        ...
        if sources:
            ...
            for source in sources:
                ...
                lines.append("")
                lines.append(
                    f"*アップロード日: {source.created_at.strftime('%Y/%m/%d') if source.created_at else '不明'}*"
                )
                lines.extend(["", "---", ""])
        ...
        if minutes:
            ...
            for minute in minutes:
                ...
                lines.append("")
                lines.append(
                    f"*作成日: {minute.created_at.strftime('%Y/%m/%d') if minute.created_at else '不明'}*"
                )
                lines.extend(["", "---", ""])
        ...
        if sessions:
            ...
            for session in sessions:
                ...
                lines.append("")
                lines.append(
                    f"*作成日: {session.created_at.strftime('%Y/%m/%d') if session.created_at else '不明'}*"
                )
                lines.append("")
                ...
                if hasattr(session, "messages") and session.messages:
                    for msg in session.messages[-10:]:  # Last 10 messages
                        ...
                        lines.append(
                            f"> {role}: {msg.content[:200]}{'...' if len(msg.content) > 200 else ''}"
                        )
                        lines.append("")
                lines.extend(["---", ""])
        ...
        if emails:
            ...
            for email in emails:
                ...
                lines.append("")
                lines.append(
                    f"*作成日: {email.created_at.strftime('%Y/%m/%d') if email.created_at else '不明'}*"
                )
                lines.extend(["", "---", ""])
